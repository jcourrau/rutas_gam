...
def astar_bidireccional(
    grafo,
    origen,
    destino,
    peso="travel_time_min",
    escala_heuristica=1.0,
):
    """Encuentra un camino mínimo mediante A* bidireccional.

    Args:
        grafo: Red dirigida con velocidades y el peso solicitado.
        origen: Nodo donde inicia la ruta.
        destino: Nodo donde finaliza la ruta.
        peso: Atributo aditivo que se desea minimizar.
        escala_heuristica: Factor no negativo aplicado a la cota temporal.

    Returns:
        Diccionario con ruta, costo, nodos explorados y tiempo de ejecución.
    """
    if escala_heuristica < 0:
        raise ValueError("escala_heuristica no puede ser negativa.")
    ...
    cola_frente = [(
        escala_heuristica * _heuristica(
            grafo, origen, destino, velocidad_maxima
        ),
        0.0,
        origen,
    )]
    cola_atras = [(
        escala_heuristica * _heuristica(
            grafo, destino, origen, velocidad_maxima
        ),
        0.0,
        destino,
    )]
    ...
    while cola_frente and cola_atras:
        ...
        for vecino, arcos in red.adj[nodo].items():
            ...
            if nuevo_costo < distancias.get(vecino, math.inf):
                ...
                padres[vecino] = nodo
                estimado = nuevo_costo + escala_heuristica * _heuristica(
